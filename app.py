...
# --- 2. FUNCIONES DE BASE DE DATOS (AHORA CON CÓDIGO) ---
def conectar_db():
...
def inicializar_db():
    ...
    """)
    # Por si la tabla ya existía, forzamos la columna codigo
...
def obtener_stock_full():
    ...
    # Agrupamos también por Código para no perderlo
...
with tab1:
    ...
    if stock_df.empty:
        ...
        if foto:
            ...
            if codigo_qr:
                # Ahora busca el código en la columna 'Código' o en el Nombre
                match = stock_df[(stock_df["Código"].astype(str) == str(codigo_qr)) | (stock_df["Producto"].str.contains(codigo_qr, case=False))]
                if not match.empty:
                    st.session_state.qr_detectado = match["Producto"].iloc[0]
                    st.success(f"✅ Encontrado: {st.session_state.qr_detectado}")
        ...
        with c4: f_search_cod = st.text_input("Código ID", placeholder="Ej: 10594") # BUSCADOR POR CÓDIGO
        ...
        if f_search_cod: df_f = df_f[df_f["Código"].astype(str).str.contains(f_search_cod, case=False)]
...
with tab4:
    ...
    if archivo and st.button("🚀 ACTUALIZAR TODO"):
        try:
            ...
            # DETECCIÓN DE COLUMNAS (Agregué Código)
            ...
            if col_prod and col_stock:
                ...
                for _, row in df_import.iterrows():
                    ...
                    codigo_val = str(row[col_cod]).strip() if col_cod else "S/C"
                    try:
                        v = str(row[col_stock]).replace('.', '').replace(',', '.')
                        stk = float(v)
                    except: stk = 0.0
                    
                    # Insertamos con Código
                    cursor.execute("INSERT OR IGNORE INTO productos (nombre, unidad, codigo) VALUES (?,?,?)", (nom, str(row[col_un]) if col_un else "LTS", codigo_val))
                ...
                st.success("✅ ¡Base de datos actualizada con códigos!")
                st.rerun()
