...
st.markdown("""
    ...
    .label-orange { background-color: #fff3cd; color: #856404; padding: 2px 6px; border-radius: 4px; font-weight: bold; }
    ...
    .comp-badge   { display: inline-block; background-color: #fd7e14; color: white; font-size: 0.65rem; padding: 1px 6px; border-radius: 8px; font-weight: bold; margin-left: 4px; vertical-align: middle; }
    </style>
...
def borrar_solo_importacion():
    conn = conectar_db()
    cursor = conn.cursor()
    cursor.execute("DELETE FROM movimientos WHERE origen = 'excel'")
...
# ---------------------------------------------------------------------------
# QUERIES CON CACHÉ (ttl=30s; limpiar con st.cache_data.clear() tras escrituras)
# ---------------------------------------------------------------------------
...
def obtener_historial_movimientos():
    conn = conectar_db()
    # FIX: columna sin acento (bug original: m.depósito)
    query = """
...
def obtener_entregas(hoja=None):
    ...
    try:
        if hoja and hoja != "Todas":
            ...
            df = pd.read_sql_query(
                "SELECT * FROM entregas ORDER BY hoja, dia_recibido DESC", conn
            )
    except:
        df = pd.DataFrame()
    conn.close()
...
# ---------------------------------------------------------------------------
# STOCK + COMPROMISOS (pendiente de entregas)
# ---------------------------------------------------------------------------
def obtener_stock_con_compromisos():
    ...
    if not entregas.empty:
        pendientes = (
            entregas[entregas["pendiente"] > 0]
            .groupby("producto")["pendiente"]
            .sum()
            .reset_index()
            .rename(columns={"producto": "Producto", "pendiente": "Comprometido"})
        )
        stock = stock.merge(pendientes, on="Producto", how="left")
    ...
    if foto_input is not None:
        try:
            ...
            img_cv = cv2.copyMakeBorder(
                img_cv, 20, 20, 20, 20, borderType=cv2.BORDER_CONSTANT, value=[255, 255, 255]
            )
...
def descargar_entregas_excel(df):
    output = io.BytesIO()
    with pd.ExcelWriter(output, engine='openpyxl') as writer:
...
with tab1:
    ...
    if stock_df.empty:
        st.warning("⚠️ No hay datos cargados. Subí el archivo en la pestaña 'Configuración'.")
    else:
        ...
        comprometidos_n = len(df_kpi[df_kpi["Disponible Neto"] < 0])

        c1, c2, c3, c4, c5, c6 = st.columns(6)
        with c1: st.metric("Total Productos",   len(df_kpi["Producto"].unique()))
        with c2: st.metric("Volumen Total",      f"{df_kpi['Stock Actual'].sum():,.0f}")
        with c3: st.metric("Stock Bajo",         criticos_n,       delta=-criticos_n,       delta_color="inverse")
        with c4: st.metric("Stock Negativo ⚠️",  negativos_n,      delta=-negativos_n,      delta_color="inverse")
        with c5: st.metric("Depósitos",          df_kpi["Deposito"].nunique())
        with c6: st.metric("Disp. Neto < 0 🔒",  comprometidos_n,  delta=-comprometidos_n,  delta_color="inverse")
        ...
        # Alerta WhatsApp si hay problemas
        wa_num = st.session_state.wa_numero
        if (negativos_n > 0 or criticos_n > 0) and wa_num:
            alertas_list = stock_df[stock_df["Stock Actual"] < U].head(15)
            msg_lines = [f"⚠️ Alerta Stock - {datetime.now().strftime('%d/%m/%Y')}"]
            for _, r in alertas_list.iterrows():
                msg_lines.append(f"• {r['Producto']}: {r['Stock Actual']:,.1f} {r['Unidad']} ({r['Deposito']})")
            wa_msg = urllib.parse.quote("\n".join(msg_lines))
            st.link_button("📱 Enviar alerta WhatsApp", f"https://wa.me/{wa_num}?text={wa_msg}")

        st.markdown("---")

        # Gráficos
        with st.expander("📊 Gráficos de Stock", expanded=False):
            col_g1, col_g2 = st.columns(2)

            with col_g1:
                df_dep = stock_df.groupby("Deposito")["Stock Actual"].sum().reset_index()
                fig_dep = px.bar(
                    df_dep.sort_values("Stock Actual", ascending=True),
                    x="Stock Actual", y="Deposito", orientation="h",
                    title="Stock Total por Depósito",
                    color="Stock Actual",
                    color_continuous_scale="Blues"
                )
                fig_dep.update_layout(showlegend=False, height=300, margin=dict(l=0, r=0, t=40, b=0))
                st.plotly_chart(fig_dep, use_container_width=True)

            with col_g2:
                top15 = (
                    stock_df.groupby("Producto")["Stock Actual"]
                    .sum()
                    .reset_index()
                ...
                fig_prod = px.bar(
                    top15.sort_values("Stock Actual", ascending=True),
                    x="Stock Actual", y="Producto", orientation="h",
                    title="Top 15 Productos por Stock",
                    color="Stock Actual",
                    color_continuous_scale="Greens"
                )
                fig_prod.update_layout(showlegend=False, height=400, margin=dict(l=0, r=0, t=40, b=0))
                st.plotly_chart(fig_prod, use_container_width=True)

            if not entregas_panel.empty:
                df_est = entregas_panel.groupby("estado").size().reset_index(name="Cantidad")
                df_est = df_est[df_est["estado"].str.strip() != ""]
                if not df_est.empty:
                    fig_est = px.pie(
                        df_est, names="estado", values="Cantidad",
                        title="Estado de Entregas",
                        hole=0.4
                    )
                    fig_est.update_layout(height=300, margin=dict(l=0, r=0, t=40, b=0))
        ...
        with c1:
            ...
            idx_inicio = lista_productos.index(st.session_state.qr_detectado) \
                if st.session_state.qr_detectado in lista_productos else 0
        ...
        with c3:
            ...
            show_comprometido = st.toggle("🔒 Solo comprometidos",    value=False)
        ...
        # FIX: lógica de toggles coherente
        if hide_neg:
            mask_pos = df_f["Stock Actual"] > 0
            mask_neg = (df_f["Stock Actual"] < 0) if show_neg_forced else pd.Series(False, index=df_f.index)
            df_f = df_f[mask_pos | mask_neg]

        if filter_reponer:
            df_f = df_f[df_f["Stock Actual"] < U]
        if show_comprometido:
        ...
        if not df_f.empty:
            ...
            if excel_bin:
                st.download_button(
                    label="📥 Descargar Comparativa Total",
                    data=excel_bin,
            ...
            for i, item in enumerate(items):
                with cols_grid[i % 4]:
                    ...
                    comp_val = item.get('Comprometido', 0)
                    disp_val = item.get('Disponible Neto', stk_val)
                    ...
                    badge_comp = '<span class="comp-badge">COMPROMETIDO</span>' if comp_val > 0 else ""
                    comp_line  = f'<br><b>🔒 Comprometido:</b> {comp_val:,.1f} | <b>Disp. Neto:</b> {disp_val:,.1f}' \
                                 if comp_val > 0 else ""
                    st.markdown(f"""
                        <div class="stock-card {clase}">
                            <div class="stock-title">{item['Producto']}{badge_neg}{badge_comp}</div>
        ...
        with st.expander("➕ Registrar movimiento manual"):
            ...
            if st.session_state.mov_pendiente is None:
                ...
                st.warning(
                    ...
                    f"- **{p['tipo']}** | **{p['producto']}** | {p['cantidad']:,.2f} | Dep: {p['deposito']}"
                )
                    ...
                    if st.button("✅ Confirmar y registrar", type="primary"):
                        ...
                        if id_p:
                            ...
                            """, (
                                ...
                                p["tipo"], id_p[0], p["cantidad"],
                                p["lote"], p["referencia"], p["deposito"], "manual"
                            ))
                            conn.commit()
...
# ===========================================================================
# FUNCIÓN REUTILIZABLE: MOSTRAR ENTREGAS
# ===========================================================================
def mostrar_tab_entregas(hoja_nombre, titulo):
    st.subheader(titulo)

    # Importador global (solo en la primera tab de entregas)
    if hoja_nombre == "LA CLEMENTINA S.A":
        with st.expander(
            "📂 Importar / Actualizar TODAS las hojas de entregas",
            expanded=obtener_entregas().empty
        ):
            st.info("Subí el archivo completo de entregas Monsanto/Bayer. Se importan las 4 hojas automáticamente.")
            arch = st.file_uploader(
                "Archivo de entregas (.xlsx)", type=["xlsx", "xls"],
                    ...
                    if df_unif.empty:
                        ...
                        for _, r in df_unif.iterrows():
                            ...
                            if descontar and r["cant_entregada"] > 0:
                                cursor.execute(
                                    "SELECT id_producto FROM productos WHERE nombre=?", (r["producto"],)
                                )
                                ...
                                if match:
                                    ...
                                    """, (
                                        ...
                                        r["lote"] or "S/L",
                                        f"Entrega a {r['cliente']}", dep_sal, "entrega"
                                    ))
                                    filas_salida += 1
                                else:
                        ...
                        conn.close()
                        guardar_metadata(
                            "ultima_importacion_entregas",
                            datetime.now().strftime("%d/%m/%Y %H:%M")
                        )
                        st.cache_data.clear()
                        msg = f"✅ {filas_ok} registros importados de las 4 hojas."
                        if descontar:
                            msg += f" {filas_salida} salidas registradas."
                        st.success(msg)
                        if no_match:
    ...
    # Pendientes vencidos (> 30 días)
    df_h["dias_pend"] = df_h["dia_recibido"].apply(dias_desde)
    vencidos_30 = len(df_h[(df_h["pendiente"] > 0) & (df_h["dias_pend"] > 30)])
    vencidos_60 = len(df_h[(df_h["pendiente"] > 0) & (df_h["dias_pend"] > 60)])
    ...
    with k6: st.metric("⏳ +30 días",   vencidos_30,  delta=-vencidos_30,  delta_color="inverse")
    with k7: st.metric("🔴 +60 días",   vencidos_60,  delta=-vencidos_60,  delta_color="inverse")

    # Alerta visual si hay pendientes muy viejos
    if vencidos_60 > 0:
        st.error(
            f"🔴 **{vencidos_60} entrega(s) con más de 60 días de antigüedad.** "
            "Revisá la tabla con filtro 'Crítico'."
        )
    elif vencidos_30 > 0:
        st.warning(f"⚠️ {vencidos_30} entrega(s) con más de 30 días pendiente.")
    ...
    with cf5:
        antigüedad_opts = ["Todos", "Normal (≤30d)", "Demorado (30-60d)", "Crítico (>60d)"]
        f_edad = st.selectbox("Antigüedad", antigüedad_opts, key=f"fedad_{hoja_nombre}")
    ...
    if not df_f2.empty:
        ...
        sub["% Entregado"] = (
            sub["Entregado"] / sub["Comprado"].replace(0, 1) * 100
        ).round(1).astype(str) + "%"
        ...
        cols_base    = ["dia_recibido", "cliente", "producto", "cantidad_comprada",
                        "cant_entregada", "pendiente", "estado", "vendedor", "dias_pend"]
        if mostrar_lote:
            cols_base.insert(3, "lote")
        ...
        excel_ent = descargar_entregas_excel(df_tabla)
        st.download_button(
            label="📥 Exportar vista filtrada a Excel",
            data=excel_ent,
            file_name=f"entregas_{hoja_nombre.replace(' ','_').replace('/','_')}.xlsx"
        )
...
with tab5:
    ...
    if not st_df.empty:
        ...
        st.download_button(
            "📥 Descargar Planilla para Conteo Físico (.xlsx)",
            data=planilla_bin,
        ...
        if st.button("💾 Guardar Auditoría de Inventario"):
            ...
            """, (
                datetime.now().strftime("%d/%m/%Y %H:%M"),
                cod_p, p_inv, d_inv, val_sistema, val_fisico, dif_inv, obs_inv
            ))
            if dif_inv != 0:
                ...
                if id_prod_match:
                    ...
                    """, (
                        datetime.now().strftime("%d/%m/%Y %H:%M"),
                        tipo_ajuste, id_prod_match[0], abs(dif_inv),
                        "S/L", f"Ajuste Inventario Físico. Obs: {obs_inv}", d_inv, "manual"
                    ))
            conn.commit()
            conn.close()
            st.cache_data.clear()
            st.success("✅ Auditoría guardada y stock ajustado.")
            st.rerun()
    else:
...
with tab6:
    ...
    if hist_df.empty:
        ...
        with c_h3:
            f_busq_h = st.text_input("🔍 Buscar", placeholder="Producto, lote, referencia...")
        ...
        with col_d2:
            fecha_hasta = st.date_input("Hasta", value=datetime.now().date(), key="hist_hasta")

        df_hf = hist_df.copy()
        if f_tipo_h != "Todos":
            df_hf = df_hf[df_hf["Tipo"] == f_tipo_h]
        if f_orig_h != "Todos":
            df_hf = df_hf[df_hf["Origen"] == f_orig_h]
        if f_busq_h:
            df_hf = df_hf[
                df_hf["Producto"].str.contains(f_busq_h, case=False, na=False) |
                df_hf["Lote"].astype(str).str.contains(f_busq_h, case=False, na=False) |
                df_hf["Referencia"].astype(str).str.contains(f_busq_h, case=False, na=False)
            ]

        # Filtro fecha (columna Fecha tiene formato %d/%m/%Y %H:%M)
        def parse_fecha_hist(s):
            try:
                return datetime.strptime(str(s)[:10], "%d/%m/%Y").date()
            except:
                return None

        df_hf["_fecha_dt"] = df_hf["Fecha"].apply(parse_fecha_hist)
        df_hf = df_hf[
            (df_hf["_fecha_dt"] >= fecha_desde) &
            (df_hf["_fecha_dt"] <= fecha_hasta)
        ]
        df_hf = df_hf.drop(columns=["_fecha_dt"])
...
with tab7:
    ...
    with st.expander("📥 Importar Archivo de Stock (CSV/Excel)", expanded=obtener_stock_full().empty):
        st.info(
            "Subí la exportación de MacroGest. "
            "Formato admitido: CSV o Excel con columnas "
            "`codigo`, `descripcion_1`, `unidad_medida`, `deposito`, `lote`, `stock_actual`."
        )
        arch_stock = st.file_uploader(
            "Archivo de Stock", type=["csv", "xlsx", "xls"], key="uploader_stock_tab7"
        )
        if arch_stock:
            if st.button("🚀 PROCESAR E IMPORTAR STOCK", type="primary"):
                try:
                    ...
                    productos_agregados = movimientos_agregados = 0
                    ...
                    for _, row in df_s.iterrows():
                        ...
                        """, (
                            datetime.now().strftime("%d/%m/%Y %H:%M"),
                            "Entrada", id_p, stk, lot, "Saldo Inicial Importado", dep, "excel"
                        ))
                    ...
                    st.cache_data.clear()
                    st.success(
                        f"✅ {productos_agregados} productos nuevos y "
                        f"{movimientos_agregados} líneas de stock impactadas."
                    )
                    st.rerun()
                except Exception as ex:
    ...
    new_wa = st.text_input(
        "Número de WhatsApp de Notificaciones (formato: 5493XXXXXXXXX)",
    ...
    with col_b1:
        if st.button(
            "🗑️ Borrar solo datos importados",
            help="Elimina los movimientos traídos por Excel; mantiene ajustes manuales"
        ):
            borrar_solo_importacion()
    ...
    with col_b2:
        confirm_borrado = st.text_input(
            "Escribí **CONFIRMAR** para habilitar el borrado total",
            key="confirm_borrado_total",
            placeholder="CONFIRMAR"
        )
        if st.button(
            "🔥 BORRAR BASE DE DATOS COMPLETA",
            type="primary",
            disabled=(confirm_borrado.strip() != "CONFIRMAR"),
            help="Borrado absoluto e irreversible de todas las tablas"
        ):
    ...
    st.markdown("---")
    st.caption("Creado por Ignacio Diaz — La Clementina S.A.")
